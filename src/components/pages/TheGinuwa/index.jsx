...
function TheGinuwa({showFooter, setShowFooter}) {
    const [index, setIndex] = useState(0)
    const [inputValue, setInputValue] = useState('')
    const {name,biography,image, start, end, span,family } = profileOfOba[index]
                ...
                <div className=" lg:w-3/5  w-full relative no-padding">
                    ...
                    <div className="flex ginuwa-crest no-margin" >
                        <div className="ss:w-1/2 p-x-24 crest-card-container w-full">
                            {
                                profileOfOba.filter((prof)=>prof.name.toLocaleLowerCase().includes(inputValue))
                                .map((profile, indexDB)=>{
